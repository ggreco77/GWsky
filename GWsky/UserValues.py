...
class SkymapCollectionMethods(object):
    ...
    def MOC_confidence_region(self, infile, percentage, short_name):
        ...
        #----------------------------------------------------------------
        # only for testing-----------------------------------------------
        samp.send_file(moc_file) # sending to Aladin plane
        #aladin.cview( url = str( params[ 'url' ]) )
...
class UserInput(SkymapCollectionMethods):
    ...
    def healpix_skymap(self):
        ...
        while True:
            try:
                ...
                print('')
                hpx = hp.read_map(self._skymap, verbose = False)
    ...
    def maximum_pixel(self):
        ...
        return self.find_highest_pixel(self._skymap)  
    ...
    def show_configure_file(self):
        ...
        for k, v in GWskyConfigure.items():
