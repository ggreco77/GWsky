...
class UserInput(SkymapCollectionMethods):
    """Insert at the command prompt user values to create the configuration files."""
    ...
    def __init__(self):
        """
            ...
            GWsky_healpix : dict to store the user values;
    ...
    def healpix_skymap(self):
        ...
        return self.GWsky_healpix.update({"skymap" : self._skymap, "prob" : prob, "nside" : nside})
    ...
    def show_configure_file(self):
        """Showing the values in the configuration file GWsky_healpix."""
        
        with open('GWsky_healpix', 'rb') as data:
            GWsky_healpix = pickle.load(data)
            
        for k, v in sorted(GWsky_healpix.items()):
            print ('GWsky_healpix:')
            print str(k) + (' : ') + str(v)
            print('')
