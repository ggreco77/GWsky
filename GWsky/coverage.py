...
class ShowSkyCoverage(Airmass):
    ...
    def vertices(self, ra_center, dec_center, fov_base, fov_height):
        """Finding the vertices of a FoV given a center position (RA[deg], DEC[deg])
           and the FOV size (FOV_base[deg], FOV_height[deg])."""
        
        ra_center_rad = radians(ra_center)
        dec_center_rad = radians(dec_center)

        fov_base_rad = radians(fov_base)
        fov_height_rad = radians(fov_height)
        ...
        arg0 = -x[0]/(cos(dec_center_rad)-y[0]*sin(dec_center_rad))
        arg1 = -x[1]/(cos(dec_center_rad)-y[1]*sin(dec_center_rad))
        arg2 = -x[2]/(cos(dec_center_rad)-y[2]*sin(dec_center_rad))
        arg3 = -x[3]/(cos(dec_center_rad)-y[3]*sin(dec_center_rad))

        v1_ra = degrees( (ra_center_rad+atan(arg0)) )
        v2_ra = degrees( (ra_center_rad+atan(arg1)) )
        v3_ra = degrees( (ra_center_rad+atan(arg2)) )
        v4_ra = degrees( (ra_center_rad+atan(arg3)) )

        v1_dec = degrees( (asin((sin(dec_center_rad)+y[0]*cos(dec_center_rad))/
                                      (1+x[0]**2+y[0]**2)**0.5)) )
        v2_dec = degrees( (asin((sin(dec_center_rad)+y[1]*cos(dec_center_rad))/
                                      (1+x[1]**2+y[1]**2)**0.5)) )
        v3_dec = degrees( (asin((sin(dec_center_rad)+y[2]*cos(dec_center_rad))/
                                      (1+x[2]**2+y[2]**2)**0.5)) )
        v4_dec = degrees( (asin((sin(dec_center_rad)+y[3]*cos(dec_center_rad))/
                                      (1+x[3]**2+y[3]**2)**0.5)) )  
