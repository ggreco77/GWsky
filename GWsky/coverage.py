...
class Airmass(object):
    ...
    def __init__(self):
        """Initializing with the input values stored in "GWsky_site"
            - observatory site - """
        
        with open('GWsky_site', 'rb') as data:
            site_GWsky = pickle.load(data)

        for k, v in site_GWsky.items():
            setattr(self, k, v)   
...
class ShowSkyCoverage(Airmass):
    ...
    def get_GWsky_healpix(self):

        with open ('GWsky_healpix', 'rb') as data:
            healpix_planes = pickle.load (data) # read parameters
                                                #--> from GWsky_healpix 
        distnorm = healpix_planes['distnorm']
        nside = healpix_planes['nside']
        distmu = healpix_planes['distmu']
        distsigma = healpix_planes['distsigma']
        prob = healpix_planes['prob']

        return distnorm, nside, distmu, distsigma, prob 

    def probability_inside_fov(self, infile, ra_vertices, dec_vertices):
         """Return the probability inside a polygon."""

         distnorm, nside, distmu, distsigma, prob = self.get_GWsky_healpix()
         ...
         probability_inside_polygon = prob[ipix_poly].sum() # probability contains
                                                        #--> in a specific FOV
    ...
    def _main_direction_funs(self, ra, dec):
        ...
        prob_fov = self.integrated_fov_probability(              # integrated FoV probability
...
class ShowSkyCoverageGUI():
    ...
    def __init__(self, tkMainWin):
        ...
        self.B32 = Button(frame, text="Start FoV", fg="red4", font=bold8)
...
class FoVstatistics(Toplevel):
    """ FoV statistics window: airmass in step of 1h, integrated probability,
            distance between 2 consecutive FoV centers, number of galaxies (distance and B mag)."""
