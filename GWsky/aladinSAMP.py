...
class AladinScriptCommands(AladinViaSAMP):
    ...
    def draw_polygon(self, x1, y1, x2, y2, x3, y3, x4, y4):
        """Draw polygon."""
     
        position = [ x1, y1, x2, y2, x3, y3, x4, y4 ]
        position = ' , '.join(map(str, position))  
        draw_polygon_str = 'draw polygon' + '( ' + position +  ')'

        return self.send_script_command(draw_polygon_str)

##    def draw_polygon(self, x1, y1, x2, y2, x3, y3, x4, y4):
##        """"Draw polygon. """
##
##        draw_polygon_str = 'draw polygon(' + x1 +','+ y1+','+ x2+','+ y2+','+ x3+','+ y3+','+ x4+','+ y4 + ')'
##
##        return self.send_script_command(draw_polygon_str)
    ...
    def cmoc(self, threshold, skymap, plane_name):
        ...
        cmoc_str = plane_name+'='+'cmoc' + ' ' + '-threshold=' + str(threshold) + ' ' + skymap + ';sync'
